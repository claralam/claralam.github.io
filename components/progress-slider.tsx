// Code from: https://cruip.com/create-a-carousel-with-progress-indicators-using-tailwind-and-nextjs/
    ...
    <div className="w-full max-w-5xl mx-auto text-center">
      ...
      <div className="transition-all duration-150 delay-300 ease-in-out">
        <div className="relative flex flex-col" ref={itemsRef}>
          ...
          {items.map((item, index) => (
            ...
            >
              <Image className="rounded-xl" src={item.img} width={7000} height={4000} alt={item.description} />
            </Transition>
          ))}
      ...
      <div className="max-w-xs sm:max-w-sm md:max-w-3xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
        ...
        {items.map((item, index) => (
          <button
            key={index}
            className="p-2 rounded focus:outline-none focus-visible:ring focus-visible:ring-amber-300 group"
          ...
          >
            <span className={`text-center flex flex-col items-center ${active === index ? '' : 'opacity-50 group-hover:opacity-100 group-focus:opacity-100 transition-opacity'}`}>
              <span className="flex items-center justify-center relative w-9 h-9 rounded-full bg-amber-100 mb-2">
                <Image src={item.buttonIcon} alt={item.description} />
              </span>
              <span className="block text-sm font-medium text-slate-900 mb-2">{item.description}</span>
              <span className="block relative w-full bg-slate-200 h-1 rounded-full" role="progressbar" aria-valuenow={active === index ? progress : 0}>
                <span className="absolute inset-0 bg-amber-500 rounded-[inherit]" style={{ width: active === index ? `${progress}%` : '0%' }}></span>
              </span>
            </span>
          </button>
        ))}
